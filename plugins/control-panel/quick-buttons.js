...
window.nova_plugins.push({
      ...
      NOVA.waitSelector('#movie_player .ytp-right-controls')
         .then(async container => {
            ...
            if (user_settings.player_buttons_custom_items?.includes('picture-in-picture')) {
               // alt1 - https://greasyfork.org/en/scripts/463641-enable-the-built-in-pip-button-on-youtube-media-control
               // alt2 - https://greasyfork.org/en/scripts/491748-for-mobile-and-desktop
               ...
               // pipBtn.append(createSvgIcon()); // fix - This document requires 'TrustedHTML' assignment.
               // pipBtn.innerHTML =
               ...
               pipBtn.addEventListener('click', () => document.pictureInPictureElement
                  ...
                  // : movie_player.togglePictureInPicture()
               );
               ...
               //    pipBtn.textContent = '';
               //    pipBtn.append(createSvgIcon(pip_enabled));
               // });
               // NOVA.videoElement?.addEventListener('leavepictureinpicture', () => {
               //    pipBtn.textContent = '';
               //    pipBtn.append(createSvgIcon());
               // });
               ...
               function createSvgIcon(pip_enabled) {
                  ...
                  // return svg; // fix - This document requires 'TrustedHTML' assignment.
            ...
            if (user_settings.player_buttons_custom_items?.indexOf('popup') !== -1 && !NOVA.queryURL.has('popup')) {
               ...
               // alt2 - https://chromewebstore.google.com/detail/gibipneadnbflmkebnmcbgjdkngkbklb
               ...
               // popupBtn.innerHTML = NOVA.createSafeHTML(
               ...
               popupBtn.addEventListener('click', () => {
                  const { width, height } = NOVA.aspectRatio.sizeToFit({
                     ...
                     // 'max_width': screen.width / (+user_settings.player_buttons_custom_popup_width || 4),
                  });
               ...
               screenshotBtn.addEventListener('click', () => {
                  ...
                  function renderSubtitle() {
                     ...
                     if (textString = document.body.querySelector('.caption-window')?.innerText) {
                        ...
                        // ctx.font = `bold ${fontSizePt}pt Arial`; //  Atention! before textWidth
                        ...
                        ctx.lineWidth = canvas.height / 1000; // ~(0.2(240p)...1(1080p))
                  ...
                  function createPreviewContainer(blob) {
                     ...
                     function createCloseButton() {
                        ...
                        // button.innerHTML = NOVA.createSafeHTML('<span>CLOSE</span>');
                        // fix - This document requires 'TrustedHTML' assignment.
            ...
            if (user_settings.player_buttons_custom_items?.includes('thumbnail')) {
               ...
               // alt6 - https://chromewebstore.google.com/detail/bjchdihmmgfbfheblpmfpaojmjchdioi
               // alt7 - https://greasyfork.org/en/scripts/449233-youtube-thumbnail-search
               // alt8 - https://greasyfork.org/en/scripts/420140
               ...
               // thumbBtn.innerHTML = NOVA.createSafeHTML(
               ...
               const thumbBtnClone = thumbBtn.cloneNode(true); // Clone with child nodes. fix - This document requires 'TrustedHTML'
            ...
            if (user_settings.player_buttons_custom_items?.includes('rotate')) {
               ...
               // alt4 - https://greasyfork.org/en/scripts/496312-youtube-com-flip-video
               const
               ...
               //    padding: '0 1.1em',
               // });
               // rotateBtn.innerHTML = NOVA.createSafeHTML(
            ...
            if (user_settings.player_buttons_custom_items?.includes('aspect-ratio')) {
               // for test - https://www.youtube.com/watch?v=xhxqDr7SqjY
               ...
               const
                  ...
                  // Solution 1. https://codepen.io/JacobLett/pen/YWeOMo
                  // aspectRatioList = [
            ...
            if (user_settings.player_buttons_custom_items?.includes('watch-later')) {
               // alt1 (Simple) - https://greasyfork.org/en/scripts/408875-youtube-watch-later-and-share-buttons-return
               // alt2 - https://openuserjs.org/scripts/zachhardesty7/YouTube_-_Add_Watch_Later_Button
               // alt3 - https://greasyfork.org/en/scripts/419656-youtube-add-watch-later-button
               // alt4 - https://greasyfork.org/en/scripts/21703-youtube-add-to-playlist-icons

               // alt (save on localStorage video queue) - https://greasyfork.org/en/scripts/462336-persistent-youtube-video-queue

               // Solution 1 (Simple). Just show default in corner
               // NOVA.css.push(
               ...
               NOVA.waitSelector('.ytp-watch-later-button')
                  .then(watchLaterDefault => {
                     ...
                     function renderIcon() {
                        // watchLaterBtn.innerHTML = NOVA.createSafeHTML(watchLaterDefault.querySelector('.ytp-watch-later-icon')?.innerHTML);
                        // fix - This document requires 'TrustedHTML' assignment.
            ...
            if (user_settings.player_buttons_custom_items?.includes('card-switch')
               ...
               // cardBtn.innerHTML = createSvgIcon();
               // cardBtn.append(createSvgIcon()); // fix - This document requires 'TrustedHTML' assignment.
               ...
               function switchState(state = required()) {
                  // cardBtn.innerHTML = createSvgIcon(state);
                  // fix - This document requires 'TrustedHTML' assignment.
                  cardBtn.textContent = ''; // clear
               ...
               function createSvgIcon(alt) {
                  ...
                  // g.innerHTML = NOVA.createSafeHTML(alt
                  //    // crossed out eye
                        ...
                        .forEach(quality => {
                           ...
                           if (qualityData = qualityFormatList[quality]) {
                              ...
                              if (badge = qualityData.badge) {
                                 ...
                                 // 50.59 % slower
                                 // qualityItem.insertAdjacentHTML('beforeend', NOVA.createSafeHTML(
                              ...
                              if (movie_player.getPlaybackQuality() == quality) {
                                 ...
                                 if ((NOVA.extractAsNum.int(qualityData.label) || 0) <= (maxWidth * 1.3)) {
                                    qualityItem.addEventListener('click', () => {
                                       ...
                                       // fix empty qualityList. onPlaybackQualityChange and addEventListener do not trigger
                                       // if (quality != 'auto') qualityList.textContent = ''; // dirty hack (clear list)
            ...
            if (user_settings.player_buttons_custom_items?.includes('clock')) {
               ...
               // clockEl.classList.add('ytp-time-display', SELECTOR_BTN_CLASS_NAME);
   ...
   options: {
      ...
      player_buttons_custom_screenshot_size: {
         ...
         options: [
            {
               label: 'video', /* value: 'video', */ selected: true, // fill value if no "selected" mark another option
               // 'label:zh': '',
               // 'label:ja': '',
               // 'label:ko': '',
               // 'label:vi': '',
